...
export const Collaborators = () => {
  ...
  const handleAddCollaborator = async () => {
    if (!newCollaborator.name.trim() || !newCollaborator.role.trim() || !newCollaborator.department.trim()) {
      toast({
        title: "❌ Campos obrigatórios",
        description: "Nome, cargo e departamento são obrigatórios.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (result) {
        console.log('✅ Colaborador criado com sucesso, atualizando interface...');
        ...
        // Aguarda um pouco e força novamente para garantir
        setTimeout(async () => {
          console.log('🔄 Segunda atualização para garantir sincronização...');
          await refetch();
        }, 1000);

        toast({
          title: "🎉 Colaborador adicionado!",
          description: `${result.name} foi cadastrado com sucesso e já aparece na lista.`,
          duration: 4000
        });
      } else {
        console.error('❌ Falha na criação do colaborador');
        toast({
          title: "❌ Erro",
      ...
      toast({
        title: "❌ Erro crítico",
        description: "Falha no sistema. Tente novamente em alguns segundos.",
        variant: "destructive"
      });
    }
  };
